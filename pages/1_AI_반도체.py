...
st.set_page_config(page_title="AI 반도체 주식 분석", page_icon="🤖", layout="wide")
...
# 사이드바
# ------------------------------------------------------------------
st.sidebar.title("🤖 AI 반도체 대시보드 설정")

selected_names = st.sidebar.multiselect(
    "분석할 종목 선택",
    options=list(AI_CHIP_TICKERS.keys()),
    default=["NVIDIA", "AMD", "TSMC (파운드리)", "SK하이닉스", "삼성전자", "ASML (노광장비)"],
)
...
st.title("🤖 AI 반도체 주식 전문 분석 대시보드")
st.caption(f"마지막 업데이트: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

tickers_to_fetch = {name: AI_CHIP_TICKERS[name] for name in selected_names}
